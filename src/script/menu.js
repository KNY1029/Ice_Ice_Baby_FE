...
window.onload = () => {
  ...
  document.querySelector('form').addEventListener('submit', e => {
    ...
    for (i = 0; i < 2; i++) { orders[orders.length-1].coffeeAdd[i] = Number(document.getElementById("coffee1-" + (i + 1)).value); };
    ...
    addCart(orders[orders.length-1]);
  });
...
function showAdd(btn) {
  ...
  switch (btn) {
    case "addBI":
      ...
      orders.push(new Order(label, 1, price));
      break;
    case "addIIB":
      label = "Ice Ice Baby"
      document.getElementById('Label').textContent = label;
      ...
      orders.push(new Order(label, 2, price));
      break;
    case "addIWB":
    ...
    case "addBIB":
      ...
      orders.push(new Order(label, 3, price));
      break;
    case "addEI":
      ...
      orders.push(new Order(label, 0, price));
      break;
    case "addET":
      label = "Extra Topping";
      document.getElementById('Label').textContent = label;
